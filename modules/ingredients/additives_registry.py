...
def get_additive_by_number(number: str) -> Optional[Dict[str, Any]]:
    """
    Get additive by E-number.

    Args:
        number: E-number (e.g., "E621", "621", "e621")

    Returns:
        Additive dictionary or None if not found
    """
    normalized = number.lower().strip()
    if not normalized.startswith("e"):
        normalized = f"e{normalized}"
    for additive in ADDITIVES:
        if additive.get("number", "").lower() == normalized:
            return additive
    return None


def get_high_risk_additives() -> List[Dict[str, Any]]:
    """
    Get all additives with risk level 3 (hazardous).

    Returns:
        List of high-risk additives
    """
    return [a for a in ADDITIVES if a.get("risk", 0) >= 3]


def get_additives_by_risk(risk_level: int) -> List[Dict[str, Any]]:
    """
    Get additives by specific risk level.

    Args:
        risk_level: 0 (safe), 1 (low), 2 (moderate), 3 (high)

    Returns:
        List of additives with matching risk level
    """
    return [a for a in ADDITIVES if a.get("risk", 0) == risk_level]


def get_additives_by_function(function: str) -> List[Dict[str, Any]]:
    """
    Get additives by function.

    Args:
        function: e.g., "color", "preservative", "sweetener", "emulsifier"

    Returns:
        List of additives with matching function
    """
    return [a for a in ADDITIVES if a.get("function", "").lower() == function.lower()]
